...
func TestRevertTo_RestoresLocalFields(t *testing.T) {
	...
	if len(n.optimizationState.schedulingOptions) != 0 {
		t.Fatalf("schedulingOptions len = %d; want 0 (truncated to [:index])", len(n.optimizationState.schedulingOptions))
	}
	if len(n.optimizationState.podTopologyDeltas) != 1 {
		t.Fatalf("podTopologyDeltas len = %d; want 1 (truncated to [:PodCount])", len(n.optimizationState.podTopologyDeltas))
	}
}
...
func TestRevertTo_ReservationIntersection_HoldDropReacquire(t *testing.T) {
	offA := reservedOfferingWithCapacity("res-A", "zone-1", 0.05, 1)
	offB := reservedOfferingWithCapacity("res-B", "zone-2", 0.05, 1)
...
func reservedOfferingWithCapacity(reservationID, zone string, price float64, cap int) *cloudprovider.Offering {
	return &cloudprovider.Offering{
		Available:           true,
